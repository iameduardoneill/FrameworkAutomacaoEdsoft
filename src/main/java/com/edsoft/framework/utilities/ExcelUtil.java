	...
	public static String setCellByName(int columnVerify, String compareName, int columnInsert, String setValue)
		...
		try {
			...
			for (int i = 1; i < totalRow; i++) {
				...
				if (ce.contains(compareName)) { /* Comparação valor e inserir valor na coluna */
					r.createCell(columnInsert).setCellValue(setValue);
					file.close();
					System.out.println("UpdateCell, Coluna de verificação:(" + columnVerify + ")- CompareName: ("
...
//			FileInputStream file = new FileInputStream(new File("date.xlsx")); /* local do arquivo */
//			XSSFWorkbook workbook = new XSSFWorkbook(file);
//			XSSFSheet sheet = workbook.getSheet("Planilha1"); /* name plan */
//				XSSFRow r = sheet.createRow(rowInsert);
//					r.createCell(columnInsert).setCellValue(setValue);
//					file.close();
//					System.out.println(
